...
<head>
	...
	<script type="text/javascript">
		$("*").ready(function () {
			...
			// $("input:hidden").val("");
			// $("input:text").val("");
			defaultCheckedBox("appliance");
			defaultCheckedBox("furniture");
			defaultCheckedBox("include");
			defaultCheckedBox("le");
			defaultCheckedBox("indetity");
			defaultRatio("shortest");
		...
		function defaultCheckedBox(cbName) {
			var deValue = $('input[name="' + cbName + '"]').val();
			var deArray = deValue.split(';');
			for (var deIndex in deArray) {
				$('input:checkbox[name="' + cbName + 'N"][value="' + deArray[deIndex] + '"]').attr("checked", true);
			}
		}
		function defaultRatio(rtName) {
			var deValue = $('#default'+rtName+'N').val();
			$('input:radio[name="' + rtName + 'N"][value="' + deValue + '"]').attr("checked", true);
		}
	...
	<div class="col_5">
		<form:form method='POST' modelAttribute="detailBean">
			<fieldset>
				...
				<div>
					每月租金
					<form:input path="price" value="${detailBean.price}" />
					<p>${errorMsg.priceE}</p>
				</div>
				<div>
					押金
				...
				<div>
					管理費
					<form:input path="hoaFee" value="${detailBean.hoaFee}" />
				</div>
				<div>
					可遷入日<input id="datepicker" name="movingInN" value="${movingInStr}" readonly="readonly"
						placeholder="請選擇日期" />
				...
				<div>
					...
					<input type="radio" name="shortestN" id="shortest2-1" value="日"><label for="shortest2-1">日</label>
